...
class SharedPreferencesHelper(context: Context) {
    ...
    // Método para incrementar contador de visitas
    fun incrementVisitCount(): Int {
        val currentCount = getInt(KEY_VISIT_COUNT, 0)
        val newCount = currentCount + 1
        saveInt(KEY_VISIT_COUNT, newCount)
        return newCount
    }

    // Método para resetear contador de visitas
    fun resetVisitCount() {
        saveInt(KEY_VISIT_COUNT, 0)
    }
